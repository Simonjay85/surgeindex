...
export async function POST(request: Request, { params }: { params: Promise<{ campaignId: string }> }) {
  ...
  if (!z.string().uuid().safeParse(campaignId).success || !body.success) return jsonError(request, 422, "invalid_payload", "A campaign and rejection reason are required.");
  ...
  try {
    const campaign = await getPostgresDb().transaction(async (tx) => {
      ...
      await tx.update(boostCampaignCreative).set({ state: "rejected", moderationReason: body.data.reason, updatedAt: new Date() }).where(eq(boostCampaignCreative.id, creative.id));
      if (["paid", "scheduled", "active"].includes(current.state)) await transitionCampaignTx(tx, campaignId, "suspended", "Admin rejected the active campaign creative.", auth.user.id, id);
      await tx.insert(adminAuditLog).values({ actorUserId: auth.user.id, action: "boost_creative_rejected", targetType: "boost_campaign", targetId: campaignId, previousState: { creative: creative.state }, newState: { creative: "rejected" }, reason: body.data.reason, requestId: id });
    ...
    return jsonError(request, 500, "moderation_failed", "The creative rejection could not be recorded.");
  }
}
