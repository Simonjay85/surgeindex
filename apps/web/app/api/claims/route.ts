...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof ClaimServiceError) return jsonError(request, error.code === "ownership_conflict" ? 409 : error.code === "site_not_found" ? 404 : 422, error.code, error.message);
