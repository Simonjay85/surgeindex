...
import { anonymousVisitorHash, hashBoostToken } from "../../../../lib/server/boost-tokens";
...
const querySchema = z.object({
  ...
  route: z.string().trim().regex(/^\/(?!api\/)[^\s]{0,510}$/).default("/"),
});

export async function GET(request: Request) {
  const url = new URL(request.url);
  const parsed = querySchema.safeParse({ placement: url.searchParams.get("placement") ?? undefined, categoryId: url.searchParams.get("categoryId") ?? undefined, route: url.searchParams.get("route") ?? undefined });
  ...
  const rate = await checkRateLimit("boost-serve", hashBoostToken(visitorHash), 120, 60 * 1000);
  if (!rate.allowed) return jsonError(request, 429, "rate_limited", "Sponsored delivery is temporarily rate-limited.");
  try {
    const result = await servedBoost({ placementKey: parsed.data.placement, categoryId: parsed.data.categoryId, routeContext: parsed.data.route, visitorContextHash: anonymousVisitorHash(request, "public"), request });
