...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return ["", "/rankings", "/breakouts", "/categories", "/live", "/radar", "/methodology", "/submit", "/privacy", "/terms", ...commercialRoutes, ...sites.map((site) => `/site/${site.slug}`)].map((path) => ({ url: `${base}${path}`, lastModified: new Date(), changeFrequency: path.includes("site/") ? "hourly" : "daily", priority: path === "" ? 1 : path.includes("site/") ? .7 : .8 }));
}
