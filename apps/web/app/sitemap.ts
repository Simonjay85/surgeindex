...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  if (env.FEATURE_CREATORS) {
    let cursor: string | undefined;
    let page = 0;
    do {
      const result = await listPublicFanwardCreators({ cursor, limit: 50 });
      entries.push(...result.creators.map((creator) => ({ url: `${base}/fanward/${creator.slug}`, lastModified: new Date(creator.publishedAt), changeFrequency: "daily" as const, priority: .7 })));
