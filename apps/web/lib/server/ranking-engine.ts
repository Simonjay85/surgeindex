      ...
      try {
        ...
        await db.transaction(async (tx) => {
          await tx
            ...
            .onConflictDoUpdate({ target: siteBaseline.siteId, set: { version: config.version, method: baseline.method, status: baseline.status, expectedVisitors: baseline.expectedVisitors, lowerBound: baseline.lowerBound, upperBound: baseline.upperBound, typicalActiveNow: baseline.typicalActiveNow, sampleCount: baseline.sampleCount, lookbackDays: baseline.lookbackDays, confidence: baseline.confidence.toFixed(4), dataCompleteness: baseline.dataCompleteness.toFixed(4), updatedAt: now } });
          for (const observation of observations) {
            await tx
              .insert(baselineBucket)
              .values({ siteId: target.siteId, bucketStart: observation.capturedAt, visitors: observation.visitors, sessions: observation.sessions, pageviews: observation.pageviews, engagedSessions: observation.engagedSessions, activeNow: observation.activeNow, validEvents: 0, dataCompleteness: "1", source: "tracker" })
              .onConflictDoUpdate({ target: [baselineBucket.siteId, baselineBucket.bucketStart], set: { visitors: observation.visitors, sessions: observation.sessions, pageviews: observation.pageviews, engagedSessions: observation.engagedSessions, activeNow: observation.activeNow, dataCompleteness: "1" } });
          }
          await tx
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        const ageSeconds = dataAgeSeconds(current?.lastAcceptedEventAt ?? null, now);
        const dataDays = await completedDataDays(db, row.siteId, now);
        ...
        const visitors24h = current?.visitors24h ?? null;
        const baselineVisitors = baseline?.expectedVisitors ?? current?.baselineDailyVisitors ?? null;
        const result = scoreSite({
          visitors24h,
          baselineDailyVisitors: baselineVisitors,
          activeNow: current?.activeNow ?? null,
          typicalActiveNow: baseline?.typicalActiveNow ?? current?.typicalActiveNow ?? null,
          engagementRate: current?.engagementRate == null ? null : number(current.engagementRate),
          avgEngagementSeconds: current?.avgEngagementSeconds ?? null,
          verification: row.verification,
          dataFreshnessSeconds: ageSeconds,
          fraudPenalty: current?.fraudPenalty == null ? 0 : number(current.fraudPenalty),
          ...
          baselineConfidence: baseline == null ? 0 : number(baseline.confidence),
          dataCompleteness: baseline == null ? 0 : number(baseline.dataCompleteness),
        ...
        await db.transaction(async (tx) => {
          ...
          if (result.components.length) {
            await tx.insert(siteScoreComponent).values(result.components.map((component) => ({ scoreId: saved.id, component: component.name, normalizedValue: component.normalizedValue.toFixed(3), weight: component.weight.toFixed(4), contribution: component.contribution.toFixed(3), available: component.available, detail: component.detail, inputValues: { visitors24h, baselineVisitors, activeNow: current?.activeNow ?? null, typicalActiveNow: baseline?.typicalActiveNow ?? null, freshness: result.freshness } })));
          }
          ...
          await tx
            .update(siteMetricCurrent)
            .set({ heatScore: result.displayedScore, rawScore: result.rawScore.toFixed(3), smoothedScore: result.smoothedScore.toFixed(3), heatLeague: result.league, rankingState: result.state, freshness: result.freshness, dataConfidence: result.confidence.toFixed(4), scoreVersion: result.version, fraudPenalty: Math.min(1, (current?.suspectedEvents24h ?? 0) / Math.max(1, (current?.acceptedEvents24h ?? 0) + (current?.suspectedEvents24h ?? 0) + (current?.invalidEvents24h ?? 0))).toFixed(3), growth24hPct: growthPct == null ? null : growthPct.toFixed(2), lastScoreAt: now, updatedAt: now })
    ...
    await db.transaction(async (tx) => {
      ...
      for (const { scope, ranked } of rankedByScope) {
        for (const [index, candidate] of ranked.entries()) {
          ...
          await tx.insert(currentRanking).values({ siteId: candidate.siteId, scope, window: RANK_WINDOW, rank, previousRank, scoreId: null, scoreVersion: config.version, displayedScore: candidate.displayedScore, smoothedScore: candidate.smoothedScore.toFixed(3), rankingState: candidate.state, league: candidate.league, generatedAt: now });
          await tx.insert(rankSnapshot).values({ siteId: candidate.siteId, scope, window: RANK_WINDOW, rank, previousRank, scoreVersion: config.version, displayedScore: candidate.displayedScore, smoothedScore: candidate.smoothedScore.toFixed(3), rankingState: candidate.state, league: candidate.league, capturedAt }).onConflictDoUpdate({ target: [rankSnapshot.siteId, rankSnapshot.scope, rankSnapshot.window, rankSnapshot.capturedAt], set: { rank, previousRank, scoreVersion: config.version, displayedScore: candidate.displayedScore, smoothedScore: candidate.smoothedScore.toFixed(3), rankingState: candidate.state, league: candidate.league } });
        }
      }
    });
