...
export function SubmitForm({ turnstileSiteKey }: { turnstileSiteKey?: string }) {
  ...
  const claimAvailable = submittedSite?.status === "active";
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const domain = safeDomain(url);
    if (!domain) { setState("error"); setMessage("Enter a public domain such as yoursite.com."); return; }
    if (!turnstileReady) { setState("error"); setMessage("Complete the anti-bot verification before submitting."); return; }
    setState("checking");
    ...
    if (response) {
      ...
      if (!response.ok) { setState("error"); setMessage(payload?.error?.message ?? "The site could not be submitted. Please try again."); }
      else if (payload?.data?.duplicate) { setState("error"); setMessage(`${domain} already has a listing. Search the directory or claim the existing profile.`); }
      else {
  ...
  return <form className="submit-card" onSubmit={submit}>
    ...
    <TurnstileField siteKey={turnstileSiteKey} action="site-submit" onToken={setTurnstileToken} onStateChange={setTurnstileState} resetNonce={turnstileResetNonce} />
    {state === "error" ? <p className="form-message form-error" role="alert">{message}</p> : null}
    {state === "success" ? <div className="form-success" role="status" aria-live="polite"><span><Check size={15} /></span><p><strong>Status: {submittedSite?.status === "pending" || submittedSite?.status === "pending_review" ? "Pending moderation" : submittedSite?.status?.replaceAll("_", " ") ?? "Submitted"}.</strong> {message}{submittedSite && !claimAvailable ? " Ownership claiming becomes available after moderation approves and activates the listing." : null}</p></div> : null}
    <div className="submit-actions">{state === "success" && submittedSite ? <div className="copy-row"><Link className="button button-coral" href={`/dashboard/sites/${submittedSite.siteId}`}>Open site dashboard <ArrowRight size={16} /></Link>{claimAvailable ? <Link className="button button-quiet" href={`/claim/${submittedSite.siteId}`}>Start ownership claim <ShieldCheck size={16} /></Link> : null}</div> : state === "success" ? <Link className="button button-coral" href="/dashboard">Open dashboard <ArrowRight size={16} /></Link> : <button className="button button-coral" type="submit" disabled={state === "checking" || !turnstileReady}>{state === "checking" ? <><LoaderCircle className="spin" size={16} /> Checking domain</> : !turnstileReady && turnstileSiteKey ? <>Verify to submit <ShieldCheck size={16} /> </> : <>Start submission <ArrowRight size={16} /> </>}</button>}<span className="form-trust"><ShieldCheck size={15} /> No tracker required to list</span></div>
  </form>;
}
