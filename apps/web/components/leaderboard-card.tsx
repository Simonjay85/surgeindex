...
function scoreStateLabel(site: DemoSite): string {
  if (site.isDemo) return "demo fixture";
  return {
  ...
  }[site.scoreState ?? "unverified"];
}

function hasOrganicScore(site: DemoSite): boolean {
  return site.isDemo || ["eligible", "provisional", "stale", "fraud_review"].includes(site.scoreState ?? "") || site.heatScore > 0;
}

export function RankSummary({ site, featured = false }: { site: DemoSite; featured?: boolean }) {
  const hasScore = hasOrganicScore(site);
...
export function LeaderboardCard({ site, featured = false }: { site: DemoSite; featured?: boolean }) {
  ...
  const hasScore = hasOrganicScore(site);
  const confidence = site.dataConfidence == null ? null : `${Math.round(site.dataConfidence * 100)}% confidence`;
  return <article className={`leaderboard-card ${featured ? "leaderboard-card-featured" : ""}`}>
    <div className="leaderboard-card-top"><div className="rank-label"><span>{site.rank > 0 && hasScore ? `#${site.rank}` : "—"}</span><RankMove movement={hasScore ? site.rankMovement : 0} /></div><span className="freshness"><span className="freshness-dot" /> {site.isDemo ? "updated 3m ago" : site.freshness === "stale" ? "data stale" : site.lastAcceptedEventAt ? "tracker measured" : "building baseline"}</span></div>
    ...
    <div className="source-row"><SourceBadge source={source} />{site.isDemo ? <DataModeBadge isDemo compact /> : null}<span className="status-chip score-state-chip">{scoreStateLabel(site)}</span>{confidence ? <span className="ownership-note">{confidence}</span> : null}<span className="ownership-note">{site.ownership === "claimed" ? "Claimed" : "Unclaimed"}</span></div>
    <div className="leaderboard-metrics">
      <div className="score-metric"><span>Organic Heat · {site.league}</span><strong>{hasScore ? site.heatScore : "—"}</strong><div className="score-meter"><span style={{ width: `${hasScore ? site.heatScore : 0}%` }} /></div></div>
...
export function CompactRankingRow({ site }: { site: DemoSite }) {
  const source = site.verification === "unverified" ? "unverified" : site.verification;
  const hasScore = hasOrganicScore(site);
  return <article className="ranking-row"><div className="row-rank">{site.rank > 0 && hasScore ? `#${site.rank}` : "—"}</div><RankMove movement={hasScore ? site.rankMovement : 0} /><div className="row-site-cell"><SiteMark site={site} size="small" /><div className="row-site"><Link href={`/site/${site.slug}`}>{site.name}</Link><span>{site.domain}</span></div></div><span className="category-chip row-category">{site.categoryName}</span><div className="row-stat"><small>Heat · {site.league}</small><strong>{hasScore ? site.heatScore : "—"}</strong></div><div className="row-stat row-visitors"><small>{site.activeNow !== null ? "Online" : "Visitors"}</small><strong>{site.activeNow !== null ? formatCount(site.activeNow) : formatCount(site.visitors)}</strong></div><div className="row-growth"><strong>{site.growthPct === null ? "—" : formatPct(site.growthPct)}</strong><small>{site.growthPct === null ? scoreStateLabel(site) : "24H growth"}</small></div><SourceBadge source={source} compact /><Sparkline values={site.sparkline} width={76} height={30} /><Link className="row-arrow" href={`/site/${site.slug}`} aria-label={`View ${site.name}`}><ArrowUpRight size={17} /></Link></article>;
}
