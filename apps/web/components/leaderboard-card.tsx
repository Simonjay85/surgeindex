...
export function RankSummary({ site, featured = false }: { site: DemoSite; featured?: boolean }) {
  return <div className={`rank-summary ${featured ? "rank-summary-featured" : ""}`}><div className="rank-number">#{site.rank}</div><RankMove movement={site.rankMovement} /><SiteMark site={site} size={featured ? "large" : "default"} /><div className="rank-summary-copy"><Link href={`/site/${site.slug}`} className="site-name-link">{site.name}</Link><span>{site.domain}</span></div><span className="category-chip">{site.categoryName}</span><div className="summary-score"><span>Heat</span><strong>{site.heatScore}</strong></div></div>;
}
