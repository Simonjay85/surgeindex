...
export function ResendVerificationForm({ turnstileSiteKey }: { turnstileSiteKey?: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError("");
    setMessage("");
    const response = await fetch("/api/auth/send-verification-email", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: email.trim(), callbackURL: `${window.location.origin}/auth/sign-in`, turnstileToken }) });
    const payload = await response.json().catch(() => null) as { error?: { message?: string } } | null;
